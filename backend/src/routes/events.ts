...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  const { token } = req as AuthRequest;
  const { contact_id, type, date, date_type, reminder_days, budget_min, budget_max } = req.body as Record<string, unknown>;
  ...
  const { data, error } = await db
    .from('events')
    .insert({ contact_id, type, date, date_type: date_type ?? 'gregorian', reminder_days: days, budget_min: budget_min ?? null, budget_max: budget_max ?? null })
    .select()
    .single();
...
router.patch('/:id', requireAuth, async (req: Request, res: Response) => {
  const { token } = req as AuthRequest;
  const { type, date, date_type, reminder_days, budget_min, budget_max } = req.body as Record<string, unknown>;
  if (reminder_days !== undefined && Number(reminder_days) < 1)
    return void res.status(400).json({ error: 'reminder_days חייב להיות לפחות 1' });
  const db = supabaseForUser(token);
  const { data, error } = await db
    .from('events')
    .update({ type, date, date_type: date_type ?? undefined, reminder_days, budget_min: budget_min ?? null, budget_max: budget_max ?? null })
    .eq('id', req.params.id)
    .select()
    .single();
