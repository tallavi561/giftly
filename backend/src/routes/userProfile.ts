...
router.get('/me', requireAuth, async (req: Request, res: Response) => {
  ...
  const { data, error } = await db
    .from('user_profiles')
    .select('user_id, display_name, nickname, email, interests, bio, gender, birth_date, city, country, privacy_level, relationship_status, has_children, religion, avatar_mode, avatar_url, created_at, updated_at')
    .eq('user_id', user.id)
    .single();
...
router.post('/', requireAuth, async (req: Request, res: Response) => {
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      avatar_mode: (avatar_mode as string) || 'illustrated', avatar_url: avatar_url || null,
    })
    .select('user_id, display_name, nickname, email, interests, bio, gender, birth_date, city, country, privacy_level, relationship_status, has_children, religion, avatar_mode, avatar_url, created_at, updated_at')
    .single();
  if (error) {
