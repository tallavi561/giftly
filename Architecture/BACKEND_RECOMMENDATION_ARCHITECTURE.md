...
**מקור סמכות יחיד:** [`Architecture/Giftly_FullSpec_ForImplementation_v1.1.pdf`](Giftly_FullSpec_ForImplementation_v1.1.pdf) (גרסה 1.1, 14 באוגוסט 2026) — מסמך זה **מאחד ומחליף** את `RecommendationArchitecture_Spec_v0.8.docx` ואת `ContactRecommendationBatch_Spec_v1.0.docx`. שני אלה, וגרסאות v0.2–v0.7 שקדמו להם, נשארים בתיקייה כארכיון היסטורי בלבד — **לא** לפנות אליהם למימוש.
...
- **לא בהיקף כלל:** frontend/UI. העיצוב הקיים (`frontend/src`, `index.css`) **לא זז**. ראו §12 להערה חשובה על שני מקומות שבהם ה-spec מרמז על צורך עתידי ב-UI חדש (פידבק בינארי לאיש קשר, שאלת-סיבה לדירוג נמוך) — מתועד כ"נדרש בעתיד", לא ממומש עכשיו.
...
| **2** | זרימת איש קשר מלאה (§5) — proven candidates, Gemini fill, pagination, session cap | Phase 1 |
| **3** | לולאת פידבק מלאה (§4 סוף, §5 binary rate) + cron צ'אנס-שני (§6) + CF batch job (§7) | Phase 1–2 |
| **4** | נתוני זרע (§9) — משימת תוכן נפרדת | Phase 1 (יכול לרוץ במקביל) |
| **5** | שיתוף אירועים/קבוצות (v1.1 spec §14, migration v19) — פיצ'ר עצמאי, לא תלוי בליבת ההמלצות | ללא תלות, יכול לרוץ בכל שלב |

מתחילים עכשיו ב-**Phase 1**.

## 12. השפעה עתידית על ה-Frontend (לא מטופלת עכשיו)

שני מקומות שבהם ה-backend החדש יחשוף יכולת שאין לה עדיין UI, בכוונה לא נבנים בפאזה הזו כדי לשמור על העיצוב הקיים כפי שהוא (ראו [`Architecture/FRONTEND_SPEC.md`](FRONTEND_SPEC.md)):

1. **פידבק בינארי על המלצה לאיש קשר** — היום `RecommendationCarousel` ב-`ContactPage` לא מציג שום כפתור דירוג. ה-API החדש (`PATCH /recommendations/:id/rate`) יהיה זמין, אבל שילוב חזותי (איפה בדיוק כפתורי "מתאים"/"לא מתאים" יושבים בכרטיס) — לדיון נפרד.
2. **שאלת-סיבה לדירוג נמוך בהמלצה עצמית** — `MyGiftsPage` כבר תומך ב-1–5 כוכבים; ה-API יידרש `feedback_reason` אופציונלי (חובה בפועל מתחת ל-4). איך/מתי שואלים את זה בפיד ה-TikTok-style — לדיון נפרד.

אלה **לא** חוסמים את פאזה 1–3 בבקאנד; ה-API פשוט יקבל `feedback_reason` כאופציונלי ולא ישבור קריאות קיימות שלא שולחות אותו.
