...
✅ **הכלי מוכן** (`backend/scripts/seedCatalog.ts`, `npm run seed:catalog` מתוך `backend/`) — מבקש מ-Gemini 25 רעיונות לכל אחת מ-12 התגיות (11 + כללי) = ~300 פריטים, מדלג על כותרות כפולות (normalized exact match, כמו שאר הדדופ במסמך), שומר `is_seed=true`, `shown=0`/`liked=0`, ומאתחל שורת אפס בכל `gift_stats_<tag>` רלוונטית. **לא הרצתי אותו** — זו פעולה שכותבת ~300 שורות ל-DB החי וצורכת קריאות Gemini אמיתיות (מחוץ למכסה היומית של המשתמשים, בכוונה), אז זו בחירה מודעת שלך מתי להריץ, לא שלי להריץ אוטומטית. המנגנון עובד גם עם קטלוג ריק בינתיים (נופל ל-exploration טהור, §4.2 מקרה קצה) — פחות איכותי, לא שבור.
...
| **3** | ✅ cron "צ'אנס שני" (§7.4) + CF batch job שמחשב `gift_neighbors` בפועל (§4.9) | Phase 1–2 |
| **4** | ✅ (כלי, לא הורץ) נתוני זרע (§10) — `npm run seed:catalog` | Phase 1 (יכול לרוץ במקביל) |
| **5** | ✅ שיתוף אירועים/קבוצות (v1.1 spec §14, migration v20) — פיצ'ר עצמאי, לא תלוי בליבת ההמלצות | ללא תלות, יכול לרוץ בכל שלב |

Phase 1–5 הושלמו קודית (2026-08-16) — הרצת ה-seed בפועל (Phase 4) נשארת בידיך; שאר הפאזות רצות live. תוכנית המימוש המקורית מוצתה — מה שנשאר מכאן הוא נתוני seed, כיוונון קבועים (נספח א׳), ו-frontend לשכבת השיתוף (לא תוכנן, ראו §12).

### הערה על Phase 5 — היקף ופשטויות מכוונות

- **בלי frontend בכלל.** ה-endpoints קיימים (`hosted-events`, `groups`, `group-invites`, `contact-invite-link`, `join/:token`) אבל שום מסך לא בנוי — תואם את מה שסוכם: זה פיצ'ר עתידי, לא חלק מהדיון הנוכחי על UX.
- **`GET /hosted-events/visible`** מממש את `getVisibleHostedEvents` מהספק (§14.3) עם 3 שאילתות + איחוד ב-JS, לא שאילתת SQL אחת — פשוט יותר לקרוא ולתחזק, ועל היקף הנתונים הצפוי כאן (כמות אירועים מתארחים לכל משתמש) ההבדל בביצועים זניח.
- **`POST /join/:token`** דורש `requireAuth` תמיד — הספק מתאר גם מקרה של לוחץ לא-רשום שמופנה להרשמה, אבל זו החלטת ניתוב/UI שאין לה עדיין מקום להתממש בו (אין frontend). כשתהיה, שם זה יטופל — ה-API עצמו נשאר "אתה חייב להיות מחובר כדי להצטרף" שזה תמיד נכון.
- **בקשות הזמנה לקבוצה לא שולחות מייל** (בניגוד ל-contact_requests, שכן שולח) — הספק לא דורש את זה, ולא הוספתי מעבר למה שהתבקש.
